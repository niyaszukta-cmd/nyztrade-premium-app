...
====================================
Two portals in one file. No external modules needed beyond streamlit.
...
def tg_renewal_reminder(telegram_id, name, days_left, plan, expiry_date):
    ...
    return _tg_send(telegram_id,
        ...
        f"Contact Dr. Niyas N to renew: 👉 https://linkedin.com/in/drniyas\n\n<i>Nyztrade Premium</i>")
...
def select_portal():
    ...
    <div style="text-align:center;padding:40px 20px 32px;">
      <div style="font-family:Outfit,sans-serif;font-size:38px;font-weight:900;color:#c084fc;letter-spacing:-1px;margin-bottom:6px;">NYZTrade Premium</div>
    ...
    <div style="display:flex;gap:20px;justify-content:center;flex-wrap:wrap;padding:0 20px 32px;">
      ...
      <div style="width:260px;background:linear-gradient(145deg,#140d24,#0d0818);border:1px solid #3b1f6b;border-radius:24px;padding:32px 28px;position:relative;overflow:hidden;box-shadow:0 4px 40px #7c3aed18;">
        ...
        <div style="font-size:12px;color:#9d8ab5;line-height:2;font-family:Space Grotesk,sans-serif;">
          ...
          <span style="color:#7c3aed;">▸</span> Revenue &amp; payment tracking<br>
    ...
    <div style="text-align:center;margin-top:24px;font-family:Space Grotesk,sans-serif;font-size:12px;color:#3b2d55;">
      Not a member yet? Contact Dr. Niyas N —
    ...
    <div style="text-align:center;font-family:Space Grotesk,sans-serif;font-size:11px;color:#2a1e40;margin-top:10px;letter-spacing:2px;text-transform:uppercase;">Dr. Niyas N &nbsp;·&nbsp; NYZTrade Premium</div>
    ''', unsafe_allow_html=True)
    ...
    <div style="background:linear-gradient(145deg,#130d22,#0e0818);border:1px solid #3b1f6b;border-radius:24px;padding:32px;position:relative;overflow:hidden;box-shadow:0 8px 48px #7c3aed20;margin-bottom:20px;">
      ...
      <div style="display:flex;align-items:center;gap:16px;margin-bottom:20px;">
        ...
        <div>
          ...
          <div style="font-family:Space Grotesk,sans-serif;font-size:11px;color:#6b5a8a;margin-top:3px;letter-spacing:1px;">Restricted Access · Dr. Niyas N only</div>
        </div>
      </div>
    </div>
    ''', unsafe_allow_html=True)
    ...
    for col, table, label in [(pc1, "equity_calls", "Equity"), (pc2, "options_calls", "Options")]:
        with col:
            ...
            if rows:
                ...
                col.info(f"No closed {label.lower()} calls yet.")
    st.divider()
                ...
                if not symbol or not entry_price or not target1 or not stop_loss:
                    ...
                    conn.execute("INSERT INTO equity_calls (symbol,call_type,entry_price,target1,target2,stop_loss,cmp,rationale,posted_date,status) VALUES(?,?,?,?,?,?,?,?,?,?)",
                        (call["symbol"], call["call_type"], call["entry_price"], call["target1"],
                         call["target2"], call["stop_loss"], call["cmp"], call["rationale"],
                         call["posted_date"], "Open"))
                    conn.commit(); conn.close()
                    msgs = []
    ...
    with tab3:
        ...
        if not rows: st.info("No closed calls yet.")
            ...
            if pnls:
                ...
                m1.metric("Win Rate", f"{wins/len(pnls)*100:.0f}%")
                m2.metric("Avg P&L",  f"{sum(pnls)/len(pnls):+.2f}%")
                m3.metric("Total",    len(rows))
            for r in rows:
            ...
            if st.form_submit_button("Post Options Call →", use_container_width=True):
                if not strike or not entry_premium or not expiry:
                    ...
                    conn.execute("INSERT INTO options_calls (underlying,option_type,strike,expiry,call_type,entry_premium,target_premium,stop_premium,cmp_premium,rationale,gex_note,posted_date,status) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",
                        (underlying, option_type, strike, expiry, call_type, entry_premium,
                         target_premium, stop_premium, cmp_premium, rationale, gex_note, str(posted_date), "Open"))
                    conn.commit(); conn.close()
                    msgs = []
...
def admin_updates():
    ...
    with tab2:
        ...
        rows = conn.execute(q, (cat_f,) if cat_f != "All" else ()).fetchall()
        conn.close()
        for r in rows:
...
def admin_videos():
    ...
    with tab2:
        ...
        rows  = conn.execute("SELECT * FROM videos" + (f" WHERE category=?" if cat_f != "All" else "") + " ORDER BY created_at DESC",
            (cat_f,) if cat_f != "All" else ()).fetchall()
        conn.close()
...
def admin_clients():
    ...
    st.markdown('<div class="section-sub">Add members, manage subscriptions, send Telegram reminders</div>', unsafe_allow_html=True)
    tab1, tab2, tab3, tab4 = st.tabs(["➕ Add Member", "👥 All Members", "💰 Log Payment", "⚠️ Expiring Soon"])

    with tab1:
        st.markdown("Members are added manually by the admin. Credentials are shared directly with each subscriber.")
        with st.form("add_member"):
            ...
            send_tg_welcome = st.checkbox("📲 Send Telegram welcome message", value=True)
                ...
                if not name or not username or not password:
                    ...
                    try:
                        conn.execute("INSERT INTO clients (name,username,password_hash,discord_id,telegram_id,email,phone,plan,status,joined_date,expiry_date,notes) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)",
                            (name, username.lower().strip(), hash_password(password), discord_id,
                             telegram_id, email, phone, plan, status, str(joined), str(expiry), notes))
                        conn.commit()
                        tg_msg = ""
                        ...
                        st.success(f"✅ {name} added | Login: **{username}** / **{password}**{tg_msg}")
                    except Exception as e:
                        st.error(f"Username already exists: {e}")
                    finally:
                        conn.close()
    ...
    with tab2:
        ...
        m3.metric("Trial",    sum(1 for r in rows if r['status'] == 'Trial'))
        m4.metric("Inactive", sum(1 for r in rows if r['status'] == 'Inactive'))
        for r in rows:
            ...
            with st.expander(f"{'🟢' if r['status']=='Active' else '🔴'} {r['name']} (@{r['username']}) | {r['plan']}{warn}"):
                ...
                if ac1.button("📲 Send Reminder", key=f"tgr_{r['id']}"):
                    if r['telegram_id']:
                        ...
                        st.warning("No Telegram ID linked for this member.")
                # Reset password
                with st.expander("🔑 Reset Password"):
                    np = st.text_input("New Password", type="password", key=f"pw_{r['id']}")
                    if st.button("Reset Password", key=f"rst_{r['id']}"):
                        conn2 = get_conn()
                        conn2.execute("UPDATE clients SET password_hash=? WHERE id=?", (hash_password(np), r['id']))
                        conn2.commit(); conn2.close(); st.success("Password reset successfully.")
                # Edit status / extend
                ...
                if ec4.button("Update", key=f"upd_{r['id']}"):
                    ...
                    conn2 = get_conn()
                    conn2.execute("UPDATE clients SET status=?,expiry_date=?,telegram_id=? WHERE id=?", (new_st, new_exp, tg_new or None, r['id']))
                    conn2.commit(); conn2.close(); st.success("Updated!"); st.rerun()
                if st.button("🗑️ Remove Member", key=f"del_{r['id']}"):
    ...
    with tab3:
        ...
        st.caption("Record payments received via UPI, bank transfer, cash, etc.")
        conn = get_conn()
        ml = conn.execute("SELECT id,name,username FROM clients ORDER BY name").fetchall()
        conn.close()
        if not ml:
            ...
            with st.form("log_payment"):
                ...
                plan   = st.selectbox("Plan", list(PLAN_AMOUNTS.keys()))
                method = st.selectbox("Payment Method", ["UPI","Bank Transfer","Cash","Card","Other"])
                pd_    = st.date_input("Payment Date", value=date.today())
                pnotes = st.text_input("Notes / Reference ID")
                ...
                if st.form_submit_button("Log Payment →", use_container_width=True):
                    if not amount:
                        ...
                        conn2 = get_conn()
                        conn2.execute("INSERT INTO payments (client_id,amount,plan,status,payment_method,notes,payment_date) VALUES(?,?,?,?,?,?,?)",
                            (cid, amount, plan, 'captured', method, pnotes, str(pd_)))
                        if extend:
                            days_map = {"Premium Monthly": 30, "Premium Quarterly": 90, "Premium Annual": 365, "Trial": 7}
                            ...
                            try:
                                ...
                                new_exp = str(date.today() + timedelta(days=30))
                            conn2.execute("UPDATE clients SET status='Active',expiry_date=?,plan=? WHERE id=?", (new_exp, plan, cid))
                        conn2.commit(); conn2.close()
                        st.success(f"✅ ₹{amount:,.0f} logged." + (f" Subscription extended to {new_exp}" if new_exp else ""))
            st.divider()
            st.markdown("#### 📋 Payment History")
            conn3 = get_conn()
            pays = conn3.execute("SELECT p.*,c.name,c.username FROM payments p LEFT JOIN clients c ON p.client_id=c.id ORDER BY p.created_at DESC").fetchall()
            conn3.close()
            if not pays:
    ...
    with tab4:
        conn = get_conn()
        rows = conn.execute("SELECT * FROM clients WHERE status='Active' AND expiry_date IS NOT NULL AND expiry_date<=date('now','+7 days') ORDER BY expiry_date").fetchall()
        conn.close()
        if not rows:
            st.success("✅ No renewals due in the next 7 days.")
        else:
            st.warning(f"⚠️ {len(rows)} member(s) expiring within 7 days!")
...
def admin_performance():
    ...
    st.markdown('<div class="section-sub">P&L charts, win rates, verified track record</div>', unsafe_allow_html=True)
    conn = get_conn()
        ...
        if not all_pnls: st.info("No closed calls yet.")
            ...
            st.markdown(f"""<div class="sub-card" style="text-align:center;padding:40px">
              ...
              <div style="display:flex;justify-content:center;gap:60px;margin-top:30px">
                <div><div style="font-size:11px;color:#445566;text-transform:uppercase;letter-spacing:2px">Total</div><div style="font-size:40px;font-weight:800;color:#fff">{len(all_pnls)}</div></div>
                <div><div style="font-size:11px;color:#445566;text-transform:uppercase;letter-spacing:2px">Wins</div><div style="font-size:40px;font-weight:800;color:#00ffb4">{tw}</div></div>
                <div><div style="font-size:11px;color:#445566;text-transform:uppercase;letter-spacing:2px">Losses</div><div style="font-size:40px;font-weight:800;color:#ff6b6b">{len(all_pnls)-tw}</div></div>
                <div><div style="font-size:11px;color:#445566;text-transform:uppercase;letter-spacing:2px">Avg P&L</div><div style="font-size:40px;font-weight:800;color:{'#00ffb4' if avg>0 else '#ff6b6b'}">{avg:+.1f}%</div></div>
              </div></div>""", unsafe_allow_html=True)
            conn = get_conn()
            pays = conn.execute("SELECT amount FROM payments WHERE status='captured'").fetchall()
            conn.close()
            if pays:
...
def member_login():
    ...
    <div style="background:linear-gradient(145deg,#130d22,#0e0818);border:1px solid #6d28d9;border-radius:24px;padding:28px 32px;position:relative;overflow:hidden;box-shadow:0 8px 48px #a855f728;margin-bottom:20px;">
      ...
      <div style="display:flex;align-items:center;gap:14px;margin-bottom:20px;">
        ...
        <div>
          <div style="font-family:Outfit,sans-serif;font-size:20px;font-weight:800;color:#fff;">Member Login</div>
      ...
      <div style="height:1px;background:linear-gradient(90deg,#6d28d9,transparent);margin-bottom:20px;"></div>
    </div>
    ''', unsafe_allow_html=True)
    ...
    with st.form("member_login"):
        ...
        if submitted:
            ...
            if member:
                ...
                st.error("Invalid credentials or subscription inactive. Contact Dr. Niyas N to subscribe.")
...
def member_home(member):
    ...
    if updates:
        ...
        st.info("No updates today yet. Check back shortly.")
    st.divider()
    left, right = st.columns(2)
    with left:
...
def member_performance(member):
    ...
    st.markdown(f"""<div class="sub-card" style="text-align:center;padding:36px">
      ...
      <div style="display:flex;justify-content:center;gap:50px;margin-top:28px;flex-wrap:wrap">
        <div><div style="font-size:11px;color:#445566;text-transform:uppercase;letter-spacing:2px">Total Calls</div><div style="font-size:38px;font-weight:800;color:#fff">{len(all_pnls)}</div></div>
        <div><div style="font-size:11px;color:#445566;text-transform:uppercase;letter-spacing:2px">Winners</div><div style="font-size:38px;font-weight:800;color:#00ffb4">{tw}</div></div>
        <div><div style="font-size:11px;color:#445566;text-transform:uppercase;letter-spacing:2px">Avg P&L</div><div style="font-size:38px;font-weight:800;color:{'#00ffb4' if avg>0 else '#ff6b6b'}">{avg:+.1f}%</div></div>
        <div><div style="font-size:11px;color:#445566;text-transform:uppercase;letter-spacing:2px">Best</div><div style="font-size:38px;font-weight:800;color:#00ffb4">{max(all_pnls):+.0f}%</div></div>
      </div></div>""", unsafe_allow_html=True)
    st.divider()
    ...
    with tab1:
        if not eq_rows: st.info("No closed equity calls yet.")
            ...
            m1, m2, m3, m4 = st.columns(4)
            m1.metric("Win Rate", f"{wins/len(pnls)*100:.0f}%"); m2.metric("Avg P&L", f"{sum(pnls)/len(pnls):+.2f}%")
            m3.metric("Best",     f"{max(pnls):+.2f}%");         m4.metric("Total",   len(pnls))
            cum = []; r = 0
            for p in pnls: r += p; cum.append(round(r, 2))
            render_line_chart("Cumulative Equity P&L (%)", [r['exit_date'] or f"#{i+1}" for i, r in enumerate(eq_rows)], cum, "#00ffb4")
            for r in eq_rows[::-1]:
...
def member_profile(member):
    ...
    st.markdown('<div class="section-sub">Your subscription details and account info</div>', unsafe_allow_html=True)
    ...
    with col_left:
        ...
        <div style="background:linear-gradient(135deg,#120d20,#0f0a1e);border:1px solid #3d1f6b;border-radius:16px;padding:24px;height:100%;">
          ...
          <div style="font-size:13px;color:#6b5a8a;margin-top:6px;font-weight:500;">@{member['username']}</div>
          {email_html}
          {discord_html}
        </div>""", unsafe_allow_html=True)
    with col_right:
    ...
    with c3:
        st.markdown(f'<div style="background:#0a0715;border:1px solid {days_border};border-radius:12px;padding:16px;text-align:center;"><div style="font-size:10px;color:#5a4870;text-transform:uppercase;letter-spacing:2px;margin-bottom:8px;">Days Left</div><div style="font-size:15px;font-weight:700;color:{dl_col};">{warn_icon}{days_label}</div></div>', unsafe_allow_html=True)
...
def main():
    ...
    if portal == "admin":
        ...
        with st.sidebar:
            st.markdown('''<div style="text-align:center;padding:20px 12px 4px;">
              <div style="font-family:Outfit,sans-serif;font-size:20px;font-weight:900;color:#c084fc;letter-spacing:-0.3px;">NYZTrade</div>
    ...
    # ── MEMBER PORTAL ─────────────────────────────────────────────────
        ...
        with st.sidebar:
            st.markdown('''<div style="text-align:center;padding:20px 12px 4px;">
              <div style="font-family:Outfit,sans-serif;font-size:20px;font-weight:900;color:#c084fc;letter-spacing:-0.3px;">NYZTrade</div>
              <div style="font-size:9px;color:#4b3a6b;letter-spacing:3px;text-transform:uppercase;margin-top:3px;">Premium Member</div>
            </div>''', unsafe_allow_html=True)
            st.divider()
            exp = date.fromisoformat(member['expiry_date']) if member.get('expiry_date') else None
            dl  = (exp - date.today()).days if exp else None
